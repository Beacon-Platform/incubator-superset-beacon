...
class PivotGridViz(BaseViz):
    ...
    def _handle_js_int_overflow(self, data):
        return self.handle_js_int_overflow(data)

    def query_obj(self):
        d = super(PivotGridViz, self).query_obj()
    ...
    def get_data(self, df):
        ...
        # Remove metrics that are not in columns
        metrics = fd.get('metrics') or []
        metrics = [self.get_metric_label(m) for m in metrics]
        groupby = fd.get('groupby')
        columns = self.form_data.get('columns')
        all_columns = metrics + groupby + columns

        for m in filter(
                lambda m: m not in all_columns, df.columns
        ):
            del df[m]

        data = self._handle_js_int_overflow(
            dict(
...
class PivotTableViz(BaseViz):
    ...
    def get_data(self, df):
        ...
        df = df.pivot_table(
            ...
            margins_name=self.form_data.get('pivot_margins_name', 'All')
        )
        # Display metrics side by side with each column
        if self.form_data.get('combine_metric'):
            df = df.stack(0).unstack()
        dark_mode = self.form_data.get('dark_mode', False)
        classes = ('dataframe table table-striped table-bordered '
                   'table-condensed table-hover table-responsive')
        if dark_mode:
            classes = ('dataframe table table-striped table-bordered '
