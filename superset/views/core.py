...
class Superset(BaseSupersetView):
    ...
    invalid_login_message = 'Access Denied.'
    ...
    @expose("/welcome")
    def welcome(self):
        """Personalized welcome page - use for autologin"""
        ...
        try:
            print('[DEBUG] Trying to login using: ', username)
            user = self.appbuilder.sm.auth_user_remote_user(username)
        except Exception as ex:
            print('[ERROR] Login failure for: %s - error: %s' % (username, str(ex)))
            user = None

        # try guest user
        if not user:
            try:
                guest = request.environ.get('GUEST_USER')
                guest_domains = request.environ.get('GUEST_DOMAINS')
                print('[DEBUG] Trying to login as guest: %s' % guest)

                if guest and guest_domains and username:
                    domain = username.split('@')[-1]
                    if not isinstance(guest_domains, (list, tuple)):
                        guest_domains = [guest_domains]
                    if domain in guest_domains:
                        print('[DEBUG] Trying to login using: ', guest, domain, guest_domains)
                        user = self.appbuilder.sm.auth_user_remote_user(guest)
            except Exception as ex2:
                print('[ERROR] Login failure, guest user not defined: %s' % str(ex2))
                user = None

        if user:
            print('[DEBUG] Login user: %s' % user)
            login_user(user)
            ...
            print('[DEBUG] Invalid user login')
            flash(as_unicode(self.invalid_login_message), 'danger')
            return self.render_template(
                'superset/invalid_login.html',
                entry='welcome',
                utils=utils)
