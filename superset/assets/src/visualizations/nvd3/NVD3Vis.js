...
const NOOP = () => {};
const formatter = _getNumberFormatter();
  ...
  const drawGraph = function () {
    ...
    if (chart.yAxis && chart.yAxis.tickFormat) {
      if (contribution || comparisonType === 'percentage') {
        ...
        yAxisFormatter = _getNumberFormatter(NumberFormats.PERCENT_1_POINT);
      }
    ...
    if (isVizTypes(['dual_line', 'line_multi'])) {
      const yAxisFormatter1 = _getNumberFormatter(yAxisFormat);
      const yAxisFormatter2 = _getNumberFormatter(yAxis2Format);
