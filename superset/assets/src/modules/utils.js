...
function __getNumberFormatter(format) {
    return function(arg) {
        var fmt = format;
        if (arg < 1 && arg > -1) {
            var x = fmt.indexOf('s');
        ...
        var t = getNumberFormatter(fmt)(arg);
        if (t) {
            return t.replace('G', 'B').replace('k', 'K').replace('m', '');
        }
        return '';
    }
};

export const _getNumberFormatter = memoize(__getNumberFormatter);

export function _formatNumber(format, num) {
    var t = _getNumberFormatter(format)(num);
    if (t) {
        return t;
    }
    return '';
...
export const _getD3Format = memoize(__getD3Format);

// Deprecated

function _3sformat(format) {
    return function(arg) {
        var fmt = table_format();
        if (arg < 1 && arg > -1) {
            var x = fmt.indexOf('s');
            if (x > -1) {
                fmt = fmt.replace('s', 'r');
